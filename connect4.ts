...
type RowType = [PlacementUnion, PlacementUnion, PlacementUnion, PlacementUnion, PlacementUnion, PlacementUnion, PlacementUnion];
type RowUnion = 1 | 2 | 3 | 4 | 5 | 6;
type AddToRow<Row extends RowUnion> = [never, 2, 3, 4, 5, 6, never][Row];
type ColumnUnion = 0 | 1 | 2 | 3 | 4 | 5 | 6;
type AddToColumn<Column extends ColumnUnion> = [1, 2, 3, 4, 5, 6, never][Column];
type SubtractFromColumn<Column extends ColumnUnion> = [never, 0, 1, 2, 3, 4, 5][Column];
...
type NewBoard<Board extends BoardType, Player extends PlayerUnion, Column extends ColumnUnion, Row extends RowUnion = 1> = [
    AddToRow<Row>,
    Board[AddToRow<Row>][Column],
...
// Tie Logic
type Tie<Board extends BoardType, Row extends RowUnion = 1, Column extends ColumnUnion = 0> = [Row] extends [never]
...
> = Count["length"] extends 4
    ...
    ? false
    : Board[Row][Column] extends Player
    ? WinningByRightDiagonal<Board, Player, AddToRow<Row>, AddToColumn<Column>, [...Count, 0]>
    : false;
...
> = Count["length"] extends 4
    ...
    ? false
    : Board[Row][Column] extends Player
    ? WinningByLeftDiagonal<Board, Player, AddToRow<Row>, SubtractFromColumn<Column>, [...Count, 0]>
    : false;
...
type Winner<Board extends BoardType, Row extends RowUnion = 3, Column extends ColumnUnion = 0> = [Row] extends [never]
    ? false
    : [Column] extends [never]
    ...
    ? WinningByRow<Board, Board[Row][Column], Row, Column> extends true
        ...
        ? Board[Row][Column]
        : WinningByRightDiagonal<Board, Board[Row][Column], Row, Column> extends true
        ? Board[Row][Column]
        : WinningByLeftDiagonal<Board, Board[Row][Column], Row, Column> extends true
        ? Board[Row][Column]
...
> = Winner<Board> extends PlayerUnion
    ...
    ? Turn extends TurnUnion
        ? Board[1][TurnToColumn<Turn>] extends Blank
            ? Connect4<RestOfTurns, NewBoard<Board, Player, TurnToColumn<Turn>>, Player extends FirstPlayer ? SecondPlayer : FirstPlayer>
